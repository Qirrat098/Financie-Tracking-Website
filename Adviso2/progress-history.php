...
$stmt = $conn->prepare("
    SELECT 
        ...
        CASE 
            WHEN g.saved_amount >= g.target_amount THEN DATE_FORMAT(g.updated_at, '%Y-%m-%d')
            ELSE NULL
...
foreach ($goals as $goal) {
    ...
    $monthly_progress[$month] += $goal['saved_amount'];

    if (!isset($category_distribution[$goal['category']])) {
        $category_distribution[$goal['category']] = 0;
    }
    $category_distribution[$goal['category']] += $goal['saved_amount'];
}
