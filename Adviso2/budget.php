...
$stmt = $conn->prepare("
    ...
    FROM (
        ...
        UNION
        SELECT 'Salary' as category
        UNION SELECT 'Freelance' as category
        UNION SELECT 'Investments' as category
        UNION SELECT 'Housing' as category
        UNION SELECT 'Utilities' as category
        UNION SELECT 'Transportation' as category
        UNION SELECT 'Food' as category
        UNION SELECT 'Entertainment' as category
        UNION SELECT 'Shopping' as category
        UNION SELECT 'Healthcare' as category
        UNION SELECT 'Education' as category
        UNION SELECT 'Travel' as category
        UNION SELECT 'Gifts' as category
        UNION SELECT 'Savings' as category
        UNION SELECT 'Other' as category
    ) as categories
    ORDER BY category
");
...
$stmt = $conn->prepare("
    ...
    LEFT JOIN transactions t ON ac.category = t.category 
        AND t.user_id = ? 
        AND DATE_FORMAT(t.transaction_date, '%Y-%m') = ?
    GROUP BY ac.category, t.type
");
$stmt->bind_param("iis", $_SESSION['user_id'], $_SESSION['user_id'], $current_month);
$stmt->execute();
$result = $stmt->get_result();
...
// If no expenses, set a default total to avoid division by zero
if ($total_expenses === 0) {
    $total_expenses = 1;
}
